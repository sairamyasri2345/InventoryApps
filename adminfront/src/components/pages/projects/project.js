...
const fetchDepts=async()=>{
  try {
    ...
    const response = await axios.get(
      "http://13.232.162.43/api/depts/depts",
      {
  ...
  const handleSave = async () => {
    ...
    try {
      if (editMode) {
        await axios.put(
          `http://13.232.162.43/api/projects/${currentProjectId}`,
          projectData
        );
      } else {
        await axios.post(
          "http://13.232.162.43/api/projects/add-project",
          projectData
        );
      }
      fetchProducts();
  ...
  const handledeptSave = async () => {
    ...
    try {
      if (editMode) {
        await axios.put(
          `http://13.232.162.43/api/depts/${currentProjectId}`,
          deptData
        );
      } else {
        await axios.post(
          "http://13.232.162.43/api/depts/add-dept",
          deptData
        );
      }
      fetchDepts();
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`http://13.232.162.43/api/projects/${id}`);
      fetchProducts();
    } catch (error) {
  ...
  const handleDeptDelete = async (id) => {
    try {
      await axios.delete(`http://13.232.162.43/api/depts/${id}`);
      fetchDepts();
    } catch (error) {
      console.error("Error deleting product:", error);
    }
